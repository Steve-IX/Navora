    ...
    <div ref={searchRef} className="w-full">
      ...
      <div className="relative mb-4">
        <input
          ...
          className="w-full px-4 py-3 pl-10 pr-10 rounded-lg border border-gray-300 dark:border-dark-border-default focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent bg-white dark:bg-dark-bg-primary text-gray-900 dark:text-dark-text-primary placeholder-gray-500 dark:placeholder-dark-text-muted"
        />
        ...
        {query && (
          <button
            ...
            className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400 dark:text-dark-text-muted hover:text-gray-600 dark:hover:text-dark-text-primary"
          >
      ...
      <div className="mb-4">
        <button
          ...
          className="w-full px-4 py-2 text-left bg-gray-100 dark:bg-dark-bg-tertiary rounded-lg hover:bg-gray-200 dark:hover:bg-dark-bg-overlay transition-colors flex items-center justify-between"
        >
          ...
          <svg
            className={`w-5 h-5 text-gray-500 dark:text-dark-text-muted transform transition-transform ${showCategories ? 'rotate-180' : ''}`}
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
        ...
        {showCategories && (
          <div className="mt-2 grid grid-cols-3 gap-2 p-2 bg-white dark:bg-dark-bg-secondary border border-gray-200 dark:border-dark-border-default rounded-lg max-h-64 overflow-y-auto">
              ...
              <button
                ...
                className={`p-3 rounded-lg text-center transition-colors ${
                  selectedCategory === category.id
                    ...
                    : 'bg-gray-50 dark:bg-dark-bg-tertiary hover:bg-gray-100 dark:hover:bg-dark-bg-overlay border-2 border-transparent'
                }`}
              >
                <div className="text-2xl mb-1">{category.icon}</div>
                <div className="text-xs font-medium text-gray-700 dark:text-dark-text-primary">{category.name}</div>
              </button>
            ))}
          </div>
        )}
      ...
      {places.length === 0 && query.length >= 2 && !isLoading && !error && (
        <div className="text-center py-8 text-gray-500 dark:text-dark-text-muted">No places found</div>
      )}
    </div>
  );
};
